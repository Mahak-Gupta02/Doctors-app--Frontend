...
const Inventory = () => {
  ...
  const getInventoryData = async () => {
    try {
      ...
      const response = await axios.get(`https://doctors-app-backend-vr89.onrender.com/getinventorydata`, {
        headers: {
          'Authorization': `${token}`
        }
      });
  ...
  const handleAdd = async(index) => {
    ...
    const token = localStorage.getItem('authToken');
    await axios.patch(`https://doctors-app-backend-vr89.onrender.com/updateinventory`, updatedInventory[index], {
      headers: {
        'Authorization': `${token}`
      }
    });
  ...
  const handleImport = async(e) => {
    ...
    reader.onload = async (event) => {
      ...
      const token = localStorage.getItem('authToken');
      await axios.post(`https://doctors-app-backend-vr89.onrender.com/bulkinventory`, {data:jsonData}, {
        headers: {
          'Authorization': `${token}`
        }
      });
